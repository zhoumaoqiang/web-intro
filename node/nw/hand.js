...
class FileListManager {
    ...
    recordList() {
        sessionStorage.handFileList = JSON.stringify(this.fileList)
    }

    showStorageList() {
        if(sessionStorage.handFileList) {
            this.fileList = JSON.parse(sessionStorage.handFileList)
            this.renderTable()
        }
    }
    ...
    getFileListStr() {
        ...
        str += `
            <tr colspan=3><td><input id=${this.name + this.ADD_LIST_BTN} type="button" value="增加列表" /></td></tr>
        `
        return str
    }
